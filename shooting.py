...
def shooting(agent):
    ...
    elif not (abs(agent.info.ball.velocity[2]) < 100 and sign(agent.team) * agent.info.ball.velocity[1] < 0):
        agent.step = "Catching"
...
def shooting_target(agent):
    ...
    backline_intersect = line_backline_intersect(agent.their_goal.center[1], vec2(car.location), vec2(car_to_ball))
    if -500 < backline_intersect < 500:
        ...
        goal_to_car = normalize(car.location - target)
...
def should_dodge(agent):
    ...
    aiming_for_goal = abs(line_backline_intersect(their_goal.center[1], vec2(car.location), vec2(car.forward()))) < 850
